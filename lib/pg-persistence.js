...
module.exports = class PgPersistence {
  ...
  async sortedTodoLists() {
    ...
    const result = await this.client.query( ALL_TODOLISTS, [this.user_id]);
  ...
  async loadTodoList(todoListId) {
    ...
    const resultTodoList = this.client.query( FIND_TODOLIST, [todoListId]);
    const resultTodos = this.client.query( FIND_TODOS, [todoListId]);
  ...
  async loadTodo(todoListId, todoId) {
    ...
    const resultTodo = await this.client.query( FIND_TODO, [todoListId, todoId]);
  ...
  async toggleDoneTodo(todoListId, todoId) {
    ...
    const updateTodo = await this.client.query( TOGGLE_TODO, [todoListId, todoId]);
  ...
  async sortedTodos(list) {
    ...
    const resultSorted = await this.client.query( SORT_TODOS, [list.id]);
  ...
  async addTodo(todoListId, title) {
    ...
    const result = await this.client.query( ADDTODO, [todoListId, title, this.user_id]);
  ...
  async addTodoList(title) {
    ...
    try {
      const result = await this.client.query( ADDTODOLIST, [title, this.user_id]);
      return result.rowCount > 0;
    } catch (error) {
  ...
  async deleteTodoList(todoListId) {
    ...
    let result = await this.client.query( DELETETODOLIST, [todoListId]);
    return result.rowCount > 0;
  }

  async deleteTodo(todoListId, todoId) {
    const DELETETODO = "DELETE FROM todos WHERE todo_list_id = $1 AND id = $2";
    let result = await this.client.query( DELETETODO, [todoListId, todoId]);
    return result.rowCount > 0;
  }
  ...
  async completeAllTodos(todoListId) {
    ...
    let result = await this.client.query( COMPLETE_ALL, [todoListId, this.user_id]);
    return result.rowCount > 0;
  }
  ...
  async existsTodoListTitle(title) {
    ...
    let result = await this.client.query( FIND_TODOLIST, [title]);
  ...
  async setTodoListTitle(todoListId, title) {
    ...
    let result = await this.client.query( UPDATE_TITLE, [title, todoListId]);
    return result.rowCount > 0;
  }

  async validateUser(username, password) {
    const FIND_USER = "SELECT * FROM users WHERE username = $1";
    const result = await this.client.query( FIND_USER, [username]);
