...
module.exports = class PgPersistence {
  ...
  async sortedTodoLists() {
    ...
    const result = await dbQuery(ALL_TODOLISTS, getClient(), this.user_id);
  ...
  async loadTodoList(todoListId) {
    ...
    const resultTodoList = dbQuery(FIND_TODOLIST, getClient(), todoListId);
    const resultTodos = dbQuery(FIND_TODOS, getClient(), todoListId);
  ...
  async loadTodo(todoListId, todoId) {
    ...
    const resultTodo = await dbQuery(FIND_TODO, getClient(), todoListId, todoId);
  ...
  async toggleDoneTodo(todoListId, todoId) {
    ...
    const updateTodo = await dbQuery(TOGGLE_TODO, getClient(), todoListId, todoId);
  ...
  async sortedTodos(list) {
    ...
    const resultSorted = await dbQuery(SORT_TODOS, getClient(), list.id);
  ...
  async addTodo(todoListId, title) {
    ...
    const result = await dbQuery(ADDTODO, getClient(), todoListId, title, this.user_id);
  ...
  async addTodoList(title) {
    ...
    try {
      const result = await dbQuery(ADDTODOLIST, getClient(), title, this.user_id);
      return result.rowCount > 0;
    } catch (error) {
  ...
  async deleteTodoList(todoListId) {
    ...
    let result = await dbQuery(DELETETODOLIST, getClient(), todoListId);
    return result.rowCount > 0;
  }

  async deleteTodo(todoListId, todoId) {
    const DELETETODO = "DELETE FROM todos WHERE todo_list_id = $1 AND id = $2";
    let result = await dbQuery(DELETETODO, getClient(), todoListId, todoId);
    return result.rowCount > 0;
  }
  ...
  async completeAllTodos(todoListId) {
    ...
    let result = await dbQuery(COMPLETE_ALL, getClient(), todoListId, this.user_id);
    return result.rowCount > 0;
  }
  ...
  async existsTodoListTitle(title) {
    ...
    let result = await dbQuery(FIND_TODOLIST, getClient(), title);
  ...
  async setTodoListTitle(todoListId, title) {
    ...
    let result = await dbQuery(UPDATE_TITLE, getClient(), title, todoListId);
    return result.rowCount > 0;
  }

  async validateUser(username, password) {
    const FIND_USER = "SELECT * FROM users WHERE username = $1";
    const result = await dbQuery(FIND_USER, getClient(), username);
