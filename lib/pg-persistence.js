...
const { dbQuery, getClient } = require("./db-query");
...
module.exports = class PgPersistence {
  ...
  async sortedTodoLists() {
    ...
    FROM todolists LEFT JOIN todos ON todolists.id = todos.todo_list_id
    WHERE todolists.user_id = $1 ORDER BY lower(todolists.title);`;
    const result = await dbQuery(ALL_TODOLISTS, this.client, this.user_id);
    console.log("RESULT ROW", result.rows);
    const convertedRecords = this.convertRecords(result.rows);
    console.log("CONVERTED RECORDS", convertedRecords);
    const partitionedLists = this.#partitionTodoLists(convertedRecords);
    console.log("PARTITIONED LISTS", partitionedLists);
  ...
  async loadTodoList(todoListId) {
    ...
    const resultTodoList = dbQuery(FIND_TODOLIST, this.client, todoListId);
    const resultTodos = dbQuery(FIND_TODOS, this.client, todoListId);
  ...
  async loadTodo(todoListId, todoId) {
    ...
    const resultTodo = await dbQuery(FIND_TODO, this.client, todoListId, todoId);
  ...
  async toggleDoneTodo(todoListId, todoId) {
    ...
    const updateTodo = await dbQuery(TOGGLE_TODO, this.client, todoListId, todoId);
  ...
  async sortedTodos(list) {
    ...
    const resultSorted = await dbQuery(SORT_TODOS, this.client, list.id);
  ...
  async addTodo(todoListId, title) {
    ...
    const result = await dbQuery(ADDTODO, this.client, todoListId, title, this.user_id);
  ...
  async addTodoList(title) {
    ...
    try {
      const result = await dbQuery(ADDTODOLIST, this.client, title, this.user_id);
      return result.rowCount > 0;
    } catch (error) {
  ...
  async deleteTodoList(todoListId) {
    ...
    let result = await dbQuery(DELETETODOLIST, this.client, todoListId);
    return result.rowCount > 0;
  }

  async deleteTodo(todoListId, todoId) {
    const DELETETODO = "DELETE FROM todos WHERE todo_list_id = $1 AND id = $2";
    let result = await dbQuery(DELETETODO, this.client, todoListId, todoId);
    return result.rowCount > 0;
  }
  ...
  async completeAllTodos(todoListId) {
    ...
    let result = await dbQuery(COMPLETE_ALL, this.client, todoListId, this.user_id);
    return result.rowCount > 0;
  }
  ...
  async existsTodoListTitle(title) {
    ...
    let result = await dbQuery(FIND_TODOLIST, this.client, title);
  ...
  async setTodoListTitle(todoListId, title) {
    ...
    let result = await dbQuery(UPDATE_TITLE, this.client, title, todoListId);
    return result.rowCount > 0;
  }

  async validateUser(username, password) {
    const FIND_USER = "SELECT * FROM users WHERE username = $1";
    const result = await dbQuery(FIND_USER, this.client, username);
