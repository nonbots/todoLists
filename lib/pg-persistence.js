...
module.exports = class PgPersistence {
  ...
  async toggleDoneTodo(todoListId, todoId) {
    ...
    const updateTodo = await this.client.query(TOGGLE_TODO, [
      todoListId,
      todoId,
    ]);
  ...
  async addTodo(todoListId, title) {
    ...
    const result = await this.client.query(ADDTODO, [
      todoListId,
      title,
      this.user_id,
    ]);
  ...
  async addTodoList(title) {
    ...
    try {
      const result = await this.client.query(ADDTODOLIST, [
        title,
        this.user_id,
      ]);
      return result.rowCount > 0;
  ...
  async completeAllTodos(todoListId) {
    ...
    let result = await this.client.query(COMPLETE_ALL, [
      todoListId,
      this.user_id,
    ]);
    return result.rowCount > 0;
